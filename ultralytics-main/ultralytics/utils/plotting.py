...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            # cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            # if label:
            #     # 使用放大后的 font scale 计算文字宽高
            #     # GC
            #     # font_scale = self.sf * 2
            #     # PCB
            #     font_scale = self.sf * 1.2
            #     w, h = cv2.getTextSize(label, 0, fontScale=font_scale, thickness=self.tf)[0]  # text width, height
            #
            #     outside = p1[1] - h >= 3
            #     # 调整填充框的位置
            #     p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
            #     cv2.rectangle(self.im, p1, p2, color, -1, cv2.LINE_AA)  # 填充框
            #
            #     # 绘制放大后的文本
            #     cv2.putText(self.im,
            #                 label,
            ...
            #     font_scale = self.sf
            #     # PCB
            #     # font_scale = self.sf * 3.5
            #     # NEU
            #     # font_scale = self.sf / 2
            #
            #     w, h = cv2.getTextSize(label, 0, fontScale=font_scale, thickness=self.tf)[0]
            #     outside = p1[1] - h >= 3
            #     # 预设文字区域（默认在上方）
            #     text_x = p1[0]
            ...
            #
            #     # 当前文字区域
            #     text_box = (box_x1, box_y1, box_x2, box_y2)
            #
            #     # 🔹 检查是否与同类标签重叠
            #     overlap_flag = False
            #     if label in self.drawn_labels:
            #         for prev_box in self.drawn_labels[label]:
